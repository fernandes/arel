...
module Arel
  module Visitors
    class MSSQL < Arel::Visitors::ToSql
      ...
      private def visit_Arel_Visitors_MSSQL_RowNumber(o, collector)
      ...
      private def get_offset_limit_clause(o)
      ...
      private def determine_order_by(orders, x)
        if orders.any?
          orders
        elsif x.groups.any?
          x.groups
      ...
      private def row_num_literal(order_by)
        RowNumber.new order_by
      end

      private def select_count?(x)
        x.projections.length == 1 && Arel::Nodes::Count === x.projections.first
      end

      # FIXME raise exception of there is no pk?
      private def find_left_table_pk(o)
      ...
      private def find_primary_key(o)
